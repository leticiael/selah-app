...
export default function AnsiedadePage() {
            ...
            <div className="space-x-4">
              {PROTOCOLS.map(p => (
                <button
                  key={p.label}
                  onClick={() => setSel(p)}
                  className="px-4 py-2 bg-white/70 text-green-900 rounded-xl hover:bg-white transition font-semibold"
                >
                  {p.label}
                </button>
              ))}
  ...
  Esta respiração (inspire por 4 segundos, expire por 6 segundos) ajuda seu corpo a relaxar e diminuir os sintomas da ansiedade. <a href="https://www.health.harvard.edu/mind-and-mood/relaxation-techniques-breath-control-helps-quell-errant-stress-response" target="_blank" rel="noopener noreferrer" className="underline">Saiba mais</a>
</p>
          </div>
        ...
        <>
          <h1 className="text-3xl font-semibold mb-4 text-green-100">
          ...
          <motion.div
            className="w-60 h-60 bg-white/40 rounded-full flex items-center justify-center text-2xl font-medium mb-6 text-green-900"
            animate={{
          ...
          <div className="fixed bottom-0 left-0 w-full">
            <div className="w-full flex flex-col md:flex-row gap-4 justify-center items-center p-4 bg-green-950/80 backdrop-blur-md">
              <button
                onClick={() => setSel(null)}
                className="px-6 py-3 text-white rounded-xl border border-white/20 hover:bg-green-800 transition"
              >
