...
export default function RaivaPage() {
            ...
            <div className="space-x-4">
              {PROTOCOLS.map(p => (
                <button
                  key={p.label}
                  onClick={() => setSel(p)}
                  className="px-4 py-2 bg-white/80 text-[#FF91AF] rounded-xl hover:bg-white transition font-semibold"
                >
                  {p.label}
                </button>
              ))}
  ...
  O fundo rosa desta página (Baker-Miller Pink) foi estudado por psicólogos e pode ajudar a acalmar emoções intensas, como a raiva. <a href="https://www.researchgate.net/publication/236843504_The_Physiological_Effect_of_Color_on_the_Suppression_of_Human_Aggression_Research_on_Baker-Miller_Pink" target="_blank" rel="noopener noreferrer" className="underline">Veja o estudo original</a>.<br />
  A respiração lenta (inspire por 4 segundos, expire por 8 segundos) também é comprovada para reduzir emoções negativas e acalmar o corpo. <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6137615/" target="_blank" rel="noopener noreferrer" className="underline">Saiba mais sobre a técnica</a>.
</p>
          </div>
        ...
        <>
          ...
          <motion.div
            className="w-60 h-60 bg-white/60 rounded-full flex items-center justify-center text-2xl font-medium mb-6 text-[#FF91AF]"
            animate={{
          ...
          <div className="fixed bottom-0 left-0 w-full">
            <div className="w-full flex flex-col md:flex-row gap-4 justify-center items-center p-4 bg-white/40 backdrop-blur-md">
              <button
                onClick={() => setSel(null)}
                className="px-6 py-3 text-black rounded-xl border border-black/30 hover:bg-black/10 transition"
              >
